...
exports.leadingBanner = leadingBanner;
 ...
 */
function stripBanner(text, kind) {
    const prefix = bannerPrefix(kind);
    const firstLine = /^[^\r\n]*/.exec(text)?.[0] ?? '';
 ...
 */
function leadingSentinelLines(kind, ucsName = '') {
...
const UCS_OPEN = new RegExp(`^[ \\t]*\\([ \\t]*function[ \\t]*(?:${IDENTIFIER})?[ \\t]*\\([ \\t]*\\)[ \\t]*\\{[ \\t]*\\r?\\n?`);
...
class MirrorFileStore {
     ...
     */
    static dedicatedBase() {
        const configured = vscode.workspace.getConfiguration('cvucsedit').get('MirrorPath', '').trim();
        if (configured) {
     ...
     */
    async resolveLocation(folderName, database) {
        ...
        const decide = async () => {
            ...
            const remembered = this.state.get(LOCATION_KEY);
    ...
    async initialize() {
        ...
        const config = vscode.workspace.getConfiguration('cvucsedit');
        const folderName = config.get('MirrorFolder', DEFAULT_MIRROR_FOLDER);
        ...
        if (this.location === 'dedicated') {
            ...
            // under them.
            const chosen = config.inspect('MirrorFolder');
            ...
            if (explicit === undefined && folderName !== LEGACY_MIRROR_FOLDER) {
                await this.migrateLegacyRoot(vscode.Uri.joinPath(base, LEGACY_MIRROR_FOLDER, sanitiseFileName(database)), vscode.Uri.joinPath(base, LEGACY_MIRROR_FOLDER));
            }
        }
     ...
     */
    async migrateLegacyRoot(legacyRoot, legacyParent) {
        try {
        ...
        try {
            ...
            this.log(`Could not move the mirror out of ${LEGACY_MIRROR_FOLDER}/ - ${error instanceof Error ? error.message : String(error)}. The old folder can be deleted by hand.`);
        }
    }
    /**
    ...
    async writeGitignore() {
        ...
        try {
            ...
            if (existing.trim() !== GITIGNORE_CONTENT.trim()) {
                this.log('.gitignore: left alone, since it is no longer the one we wrote. Remove the ' +
                    "'*' line by hand if you meant to track the mirror in git.");
                return;
            }
            await vscode.workspace.fs.delete(uri);
            this.log('.gitignore: removed (cvucsedit.WriteMirrorGitignore is off). Git now sees this ' +
     ...
     */
    async writeRootPointer(block, merge, requestConsent) {
        if (!this.pointerRoot) {
            return;
        }
    ...
    async writePointerBlock(root, block, merge, requestConsent) {
        ...
        for (const file of planned) {
            try {
                ...
                this.log(`${file.name}: could not write the UCS pointer block - ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }

...
(function (dataTypeCheck) {
    ...
    dataTypeCheck[dataTypeCheck["number"] = 1] = "number";
    dataTypeCheck[dataTypeCheck["bool"] = 2] = "bool";
    dataTypeCheck[dataTypeCheck["text"] = 3] = "text";
    dataTypeCheck[dataTypeCheck["noCheck"] = 4] = "noCheck";
...
class ucsmValidation {
    ...
    getUCSJSLineWithoutComment(line, inMultiLineComment) {
        let trimStart = 0;
    ...
    validateUCSJS(text, langID) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!evalRusult.valid) {
                this.addDiagnostic(diagnostics, i, startOffset, startOffset + lineToEval.length, evalRusult.notValidText);
            }
        }
        return diagnostics;
    }
                        ...
                        if (!conditionMatch) {
                            ...
                            if (!condition) {
                                ...
                                if (!evalRusult.valid) {
                                    ...
                                    this.addDiagnostic(diagnostics, i, startOffset + structure.openingKeyword.length + 1, lineToEval.length - structure.requiredSuffix.length - 1 - startOffset, evalRusult.notValidText
            ...
            if (lineToEval.match(/^[A-Za-z_{}:][A-Za-z0-9_:<>{}@\.]*\s*(?:=|\:=|=[\+\-\*\/]=)/i)) {
                ...
                if (assignmentMatch) {
                    ...
                    if (!evalRusult.valid) { //isValidCondition
                        this.addDiagnostic(diagnostics, i, startOffset, lineToEval.length - startOffset, evalRusult.notValidText
    ...
    getFunctionDataType(condPart) {
        for (const Func of this.functions) {
            const functionRegex = new RegExp(`${Func.name}\\(?$`, 'gi');
            if (functionRegex.test(condPart)) {
                const inputDataType = dataTypeCheck[Func.inputType];
                const returnDataType = dataTypeCheck[Func.returnType];
                return [Func.name, inputDataType, returnDataType];
            }
        }
        //return false;
    }
    EvalStatement(statement, mustBeComparision) {
        ...
        //     }
        // }
        function CompareDataType(thisType, CompareType) {
        ...
        function setDataTypeFromPattern(part) {
        ...
        function addWhiteSpaceWhenRequired(part) {
            let newPart = '';
        ...
        function getClosingBlockIndex(lineText, blockChar, startIndex) {
            //console.log(`startIndex "${startIndex}" lineText "${lineText.substring(startIndex)}"`);
            if (blockChar == `'`)
                return lineText.substring(startIndex).indexOf(`'`);
            let openCount = 0;
        ...
        const evalBlock = (evalText) => {
            ...
            while (i < evalText.length) {
                ...
                if (char == '(') {
                    let blockResult;
                    let funcName = '';
                    let inputDataType, returnDataType = dataTypeCheck.any;
        ...
        const evalPart = (partToEval, evalDataType, expectDataType) => {
            function handleReturnType(returnType) {
                const compareResult = CompareDataType(returnType, evalDataType || dataTypeCheck.noCheck);
                if (!compareResult)
                    return { valid: false, notValidText: `Invalid data type match.` };
            ...
            let checkDataType = dataTypeCheck.any;
    ...
    checkBalancedBrackets(lineText) {
        // ,
        // {
        //   "opening": "<",
        //   "closing": ">",
